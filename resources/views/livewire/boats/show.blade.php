...
//    #[\Livewire\Attributes\Rule(['$repairImages.*' => 'image|sometimes'])]
    ...
    public function loadYoutubePlaylist()
    {
        ...
        $list = $youtube->getPlaylistItems($this->model->discipline->playlist);
...
<div>
    ...
    <x-mary-header title="{{$boatRegistration->boat->model}}">
        <x-slot:actions>
            <x-mary-button label="Remove boat" icon="o-trash" class="btn-error" wire:click="$dispatch('showDelete')"
    ...
    <div class="mb-5 gap-5 {{ $notComplete?'blur-sm':'' }}">
        <x-mary-button label="Boat care" icon="tabler.shield-heart"
                       class="lg:btn-lg btn-secondary w-full mb-2 lg:w-auto lg:mb-0"
                       wire:click="loadContent({{ Content::findByPath('boatcare')->id }})" spinner></x-mary-button>
        <x-mary-button label="Tips and Tricks" icon="tabler.video"
                       class="lg:btn-lg btn-secondary w-full mb-2 lg:w-auto lg:mb-0" wire:click="loadYoutubePlaylist()"
                       spinner></x-mary-button>
        <x-mary-button label="Repair" icon="o-wrench-screwdriver"
                       class="lg:btn-lg btn-secondary w-full mb-2 lg:w-auto lg:mb-0" @click="$wire.showRepair = true"
                       spinner></x-mary-button>
        <x-mary-button label="Move this boat" icon="o-truck" link="https://moveyourboat.paddle-lab.com"
                       class="lg:btn-lg btn-secondary w-full mb-2 lg:w-auto lg:mb-0" external></x-mary-button>
        <x-mary-button label="Sell" icon="o-currency-euro" class="lg:btn-lg btn-neutral w-full mb-0 lg:w-auto"
                       wire:click="loadContent({{ Content::findByPath('sell')->id }})" spinner></x-mary-button>
    </div>
    ...
    <div class="grid lg:grid-cols-3 gap-5">
            ...
            @foreach($this->details as $detail)
                <x-mary-list-item :item="$detail" sub-value="sub-value" class="{{ $detail['class']??'' }}">
                    ...
                    @if($detail['sub-value'] == 'Boat model' && isset($aboutModel))
                        <x-slot:actions>
                            <x-mary-button icon="o-information-circle" class="btn-circle btn-sm"
                                           wire:click="loadContent({{ $aboutModel->id }})" spinner></x-mary-button>
                        </x-slot:actions>
                    @endif
                    @if($detail['sub-value'] == 'Market value')
                        <x-slot:actions>
                            <x-mary-button icon="o-information-circle" class="btn-circle btn-sm"
                                           wire:click="loadContent({{ Content::findByPath('marketvalue')->id }})"
                                           spinner></x-mary-button>
                        </x-slot:actions>
                    @endif
                </x-mary-list-item>
            ...
            @if($boatRegistration->status == \App\Enums\StatusEnum::VALIDATED)
                <div class="align-middle self-center space-y-3">
                    @if(empty($boat->finished_at))
                        <p>Your boat is yet not finished. As soon as that happens, you will have access to all the
                        ...
                        @if(!$boat->voucher_used)
                            ...
                            @if(empty($boatRegistration->voucher))
                                <p>You will need a voucher code to complete this operation. Please request one to your
                                    dealer.</p>
                                <x-mary-form wire:submit="saveVoucher" class="lg:w-1/3">
                                    <x-mary-input label="Voucher code" wire:model="voucher" inline>
                                        <x-slot:append>
                                            <x-mary-button label="Send" type="submit" spinner
                                                           icon="o-paper-airplane"></x-mary-button>
                                        </x-slot:append>
                                    </x-mary-input>
                                </x-mary-form>
                            @else
                                <x-mary-button label="Get my goodies" icon="o-sparkles"
                                               link="https://www.nelo.eu/pre-registration-options/?{{ $voucherQueryString }}"
                                               external></x-mary-button>
                            @endif
                        @endif
                    @else
                        <h2 class="mb-3">Please complete your boat registration</h2>
                        <x-mary-button label="Finish registration" icon="o-adjustments-horizontal"
                                       @click="$wire.showSetup = true"></x-mary-button>
                    @endif
                </div>
            @else
                <x-slot:menu>
                    <x-mary-button icon="o-pencil-square" class="btn-circle btn-sm"
                                   wire:click="$toggle('showSetup')"></x-mary-button>
                </x-slot:menu>
                @isset($discipline)
                                ...
                                @case(FieldEnum::Seat)
                                    @isset($boatRegistration->seat)
                                        <x-mary-list-item :item="$boatRegistration->seat">
                                            ...
                                            <x-slot:avatar>
                                                <x-mary-avatar :image="$boatRegistration->seat->image"
                                                               class="!w-11"></x-mary-avatar>
                                            </x-slot:avatar>
                                        </x-mary-list-item>
                                    @endisset
                                    @break
                                ...
                                @case(FieldEnum::SeatPosition)
                                    @isset($boatRegistration->seat_position)
                                        <x-mary-list-item :item="$boatRegistration" value="seat_position">
                                            ...
                                            <x-slot:avatar>
                                                <x-mary-icon name="tabler.arrow-autofit-width"
                                                             class="!w-11"></x-mary-icon>
                                            </x-slot:avatar>
                                        </x-mary-list-item>
                                    @endisset
                                ...
                                @case(FieldEnum::SeatHeight)
                                    @isset($boatRegistration->seat_height)
                                        <x-mary-list-item :item="$boatRegistration" value="seat_height">
                                            ...
                                            <x-slot:avatar>
                                                <x-mary-icon name="tabler.arrow-autofit-height"
                                                             class="!w-11"></x-mary-icon>
                                            </x-slot:avatar>
                                        </x-mary-list-item>
                                    @endisset
                                ...
                                @case(FieldEnum::Footrest)
                                    @isset($boatRegistration->footrest)
                                        <x-mary-list-item :item="$boatRegistration->footrest">
                                            ...
                                            <x-slot:avatar>
                                                <x-mary-avatar :image="$boatRegistration->footrest->image"
                                                               class="!w-11"></x-mary-avatar>
                                            </x-slot:avatar>
                                        </x-mary-list-item>
                                    @endisset
                                ...
                                @case(FieldEnum::FootrestPosition)
                                    @isset($boatRegistration->footrest_position)
                                        <x-mary-list-item :item="$boatRegistration" value="footrest_position">
                                            ...
                                            <x-slot:avatar>
                                                <x-mary-icon name="tabler.arrow-bar-both" class="!w-11"></x-mary-icon>
                                            </x-slot:avatar>
                                        </x-mary-list-item>
                                    @endisset
                                ...
                                @case(FieldEnum::Rudder)
                                    ...
                                    @isset($boatRegistration->rudder)
                                        <x-mary-list-item :item="$boatRegistration->rudder">
                                            <x-slot:sub-value>Rudder</x-slot:sub-value>
                                            <x-slot:avatar>
                                                <x-mary-avatar :image="$boatRegistration->rudder->image"
                                                               class="!w-11"></x-mary-avatar>
                                            </x-slot:avatar>
                                        </x-mary-list-item>
                                    @endisset
        ...
        <x-mary-card title="Fittings" @class(['blur-sm' => $notComplete])>
            <x-slot:menu>
                <x-mary-button label="" icon="o-squares-plus" @click="$wire.showComponents = true;"
                               class="btn-circle btn-sm" tooltip="View all components"></x-mary-button>
            </x-slot:menu>
            @foreach($fittings as $product)
                <x-mary-list-item :item="$product" sub-value="attribute.name">
                    <x-slot:avatar>
                        <x-mary-avatar :image="$product->image" class="!w-11"></x-mary-avatar>
                    </x-slot:avatar>
                    <x-slot:actions>
                        @isset($product->attribute)
                            <x-mary-button label="Upgrade" wire:click="loadUpgrades({{ $product->attribute->id }})"
                                           spinner></x-mary-button>
                        @endisset
                    </x-slot:actions>
                </x-mary-list-item>
            ...
            @isset($boatRegistration->boat->assembler)
                <div class="mt-10">
                    <x-mary-avatar :title="$boatRegistration->boat->assembler->name" subtitle="Assembly"
                                   :image="$boatRegistration->boat->assembler->photo" class="!w-14"></x-mary-avatar>
                </div>
            @endisset
            @isset($boatRegistration->boat->evaluator)
                <div class="mt-10">
                    <x-mary-avatar :title="$boatRegistration->boat->evaluator->name" subtitle="Quality control"
                                   :image="$boatRegistration->boat->evaluator->photo" class="!w-14"></x-mary-avatar>
                </div>
            @endisset
        </x-mary-card>
        ...
        <x-mary-card title="Design" @class(['blur-sm' => $notComplete])>
            @foreach($colors as $product)
                <x-mary-list-item :item="$product" sub-value="attribute.name">
                    <x-slot:avatar>
                        <div style="background-color: {{ $product->attributes['hex']??'#ffffff' }};"
            ...
            @if(!empty($boatRegistration->boat->painter))
                <div class="mt-10">
                    <x-mary-avatar :title="$boatRegistration->boat->painter->name" subtitle="Painter"
                                   :image="$boatRegistration->boat->painter->photo" class="!w-14"></x-mary-avatar>
                </div>
            @endif
        </x-mary-card>
        ...
        <x-mary-card title="Layup" @class(['blur-sm' => $notComplete])>
            ...
            @if(!empty($boatRegistration->boat->layuper))
                <div class="mt-10">
                    <x-mary-avatar :title="$boatRegistration->boat->layuper->name" subtitle="Layup"
                                   :image="$boatRegistration->boat->layuper->photo" class="!w-14"></x-mary-avatar>
                </div>
            @endif
        </x-mary-card>
    </div>
    ...
    <x-mary-drawer wire:model="showSetup" right class="w-11/12 lg:w-1/3">
        <x-mary-form wire:submit="saveSetup">
            @foreach($setupFields as $field)
                @switch(FieldEnum::from($field->id))
                    @case(FieldEnum::Seat)
                        <x-mary-select label="Seat" wire:model="seat_id"
                                       :options="$model->options()->where('product_type_id', ProductTypeEnum::Seat)->get()"
                                       placeholder="---"></x-mary-select>
                        @break
                    ...
                    @case(FieldEnum::Footrest)
                        <x-mary-select label="Footrest" wire:model="footrest_id"
                                       :options="$model->options()->where('product_type_id', ProductTypeEnum::Footrest)->get()"
                                       placeholder="---"></x-mary-select>
                        @break

                    @case(FieldEnum::FootrestPosition)
                        <x-mary-range label="Footrest Position" wire:model="footrest_position" min="0"
                                      max="20"></x-mary-range>
                        <x-mary-badge x-text="$wire.footrest_position">{{$footrest_position}}</x-mary-badge>
                        @break

                    @case(FieldEnum::Rudder)
                        <x-mary-select label="Rudder" wire:model="rudder_id"
                                       :options="$model->options()->where('product_type_id', ProductTypeEnum::Rudder)->get()"
                                       placeholder="---"></x-mary-select>
                        @break
    ...
    <x-mary-drawer wire:model="showUpgrades" title="Available upgrades" right class="w-11/12 lg:w-1/3">
        @isset($upgradables)
            @foreach($upgradables as $option)
                <x-mary-list-item :item="$option">
                    ...
                    <x-slot:avatar>
                        <x-mary-avatar :image="$option->image" class="!w-11"></x-mary-avatar>
                    </x-slot:avatar>
                    @isset($option->attributes['magento_url'])
                        <x-slot:actions>
                            <x-mary-button label="Buy"
                                           :link="config('nelo.shop.base_product_url').$option->attributes['magento_url'].'.html'"
                                           external></x-mary-button>
                        </x-slot:actions>
                    @endisset
                </x-mary-list-item>
        ...
        <x-mary-button icon="o-beaker"
                       link="https://paddle-lab.com/paddles-accessories/https-paddle-lab-com-paddles-accessories-nelo-training-gadgets-html.html"
                       external>Discover our training gadgets
        </x-mary-button>
        <x-slot:actions>
    ...
    <x-mary-modal title="Repair service"
                  subtitle="Fill the form bellow so our team can analyze it and send you a quote."
                  wire:model="showRepair">
        @push('head')
            {{-- Cropper.js --}}
        ...
        <x-mary-form wire:submit="sendRepair">
            <x-mary-textarea label="Describe the issues with your boat" rows="5" wire:model="repairDescription"
                             required></x-mary-textarea>
            <x-mary-image-library label="Photos of the issues" wire:model="repairImages" wire:library="repairLibrary"
                                  :preview="$repairLibrary" required></x-mary-image-library>
            <x-slot:actions>
    ...
    <!-- YOUTUBE PLAYLIST -->
    <x-mary-drawer wire:model="showPlaylist" title="Tips and Tricks" right class="w-11/12 lg:w-1/2">
        @foreach($playlist as $item)
            <x-mary-list-item :item="$item" sub-value="description" class="text-xl no-tr">
                <x-slot:avatar>
        ...
        @endforeach
        <x-slot:actions>
            <x-mary-button label="Close" @click="$wire.showPlaylist = false"></x-mary-button>
        </x-slot:actions>
    </x-mary-drawer>
    <x-mary-modal wire:model="showYoutube" box-class="w-auto max-w-full">
        <iframe id="ytplayer" type="text/html" width="640" height="360"
                src="{{ $youtubeVideo }}?fs=1"
                frameborder="0"></iframe>
        <x-slot:actions>
            <x-mary-button label="Close" @click="$wire.showYoutube = false"></x-mary-button>
        </x-slot:actions>
    </x-mary-modal>
</div>
